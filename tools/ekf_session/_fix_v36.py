# -*- coding: utf-8 -*-
"""VER=35 -> 36：给磁偏航环加**增益限幅**（等价于地磁牵引的 τ 下限）。

病：实测静止稳、缓慢抬板边就"自行震动"（CH4~7 旧链稳定作对照）。
    机制：K = P[8][8]/(HPH'+R)。我为了让 sigma_tilt 诚实，在 Q_tt 里加了整流项
    (0.85*|a_lin|)^2*dt。静止 |a_lin|~0.05g -> K~0.06（轻推，稳）；
    缓慢运动 |a_lin| 到 0.3~1g -> P[8][8] 被抬高 -> K 冲到 0.5~0.9
    -> 磁观测由"轻推"变成"每周期硬拉"，而 IST8310 是 187 Hz、单样本方位噪声
    0.5~0.7 度 -> 偏航逐样本跟随噪声 = 看到的震动。
    ★ 根子：把"姿态该有多不确定"和"这个观测该有多少权重"绑在了一起。
      诚实的大方差不该让一个噪声大的观测获得高增益。

修：每周期对偏航的修正量限幅 -> 磁环变成有 τ 下限的**牵引**，噪声被压平，
    而真实的陀螺零偏漂移（0.01~0.1 dps）远在限幅速率之内，照拉无误。
"""
import re
import shutil

R = r'C:\Users\33\Documents\v2\h415-imu42605-'
P = R + r'\V5F\User\src\proc_ekf.c'
T = R + r'\V5F\User\inc\v5f_tune.h'


def sw(path, text, enc, tag):
    data = text.encode(enc)
    shutil.copy2(path, path + '.bak_' + tag)
    with open(path, 'wb') as f:
        f.write(data)


u = open(T, 'rb').read().decode('gbk')
a = '#define V5F_EKF_NIS_MAX_MAG     100.0f'
assert u.count(a) == 1
u = u.replace(a, a + """
/* 磁偏航环的**单周期修正上限**（度）。等价于给地磁牵引一个 τ 下限：
 *   tau_min = dt_e / (STEP*DEG2RAD) = 2.87ms / (0.05*0.01745) = 3.3 s
 * 动机：K = P[8][8]/(HPH'+R)，而 Q_tt 里的整流项在运动中把 P[8][8] 抬高，
 * 使 K 从 0.06 冲到 0.9 -> 磁观测由"轻推"变成"每周期硬拉"，
 * 偏航于是逐样本跟随 IST8310 的 0.5~0.7 度噪声 = 曲线上的自行震动。
 * 限幅后噪声被压平，而真实陀螺零偏漂移（0.01~0.1 dps = 最大 0.1 度/周期）
 * 仍在限幅速率（0.05 度/周期 = 17 度/s）之内，照拉无误。 */
#define V5F_EKF_YAW_STEP_MAX_DEG 0.05f""", 1)
assert u.count('#define V5F_FW_VER        35u') == 1
u = u.replace('#define V5F_FW_VER        35u', '#define V5F_FW_VER        36u', 1)
assert u.count('/*') == u.count('*/')
sw(T, u, 'gbk', 's2e')
print('v5f_tune.h: YAW_STEP_MAX_DEG + VER 36')

t = open(P, 'rb').read().decode('gbk')

# 加一个"取四元数偏航"的小工具
a = 'static void H_zero(uint8_t m)'
assert t.count(a) == 1
t = t.replace(a, """/* 四元数的导航系偏航角（与 report 里的定义一致） */
static float q_yaw(const float *q)
{
    return atan2f(2.0f*(q[0]*q[3] + q[1]*q[2]), 1.0f - 2.0f*(q[2]*q[2] + q[3]*q[3]));
}

static void H_zero(uint8_t m)""", 1)

# M7：更新前后比偏航，超限就转回去
a = """        r[0] = wrap_pi(V5F_MAG_DECL_RAD - hh);
        st = ekf_update(R, 1u, r, V5F_EKF_NIS_MAX_MAG, &s_nis[4], &s_rej[3], 0x0100u);
        if (st == 0u) s_gate_bits |= V5F_EKF_GB_MAG;"""
assert t.count(a) == 1, t.count(a)
b = """        r[0] = wrap_pi(V5F_MAG_DECL_RAD - hh);
        {
            /* ★ 增益限幅：本周期对偏航的修正不得超过 V5F_EKF_YAW_STEP_MAX_DEG。
             *   没有它，运动中被整流项抬高的 P[8][8] 会让 K 冲到 0.9，
             *   磁观测变成"每周期硬拉"，偏航就逐样本跟随 IST8310 的 0.5~0.7 度
             *   噪声 —— 实测就是静止稳、缓慢抬板边自行震动，而旧链（纯积分+
             *   低增益倾斜修正）完全稳定。限幅后它是一个有 tau 下限的**牵引**。 */
            float y0 = q_yaw(&s_x[IX_Q]);
            float y1, dlt, lim = V5F_EKF_YAW_STEP_MAX_DEG * DEG2RAD;
            st = ekf_update(R, 1u, r, V5F_EKF_NIS_MAX_MAG, &s_nis[4], &s_rej[3], 0x0100u);
            y1 = q_yaw(&s_x[IX_Q]);
            dlt = wrap_pi(y1 - y0);
            if (dlt > lim || dlt < -lim) {
                float corr = dlt - ((dlt > 0.0f) ? lim : -lim);   /* 超出的部分 */
                float hq[4], qt[4];
                uint32_t n2;
                hq[0] = cosf(-0.5f * corr); hq[1] = 0.0f; hq[2] = 0.0f;
                hq[3] = sinf(-0.5f * corr);
                q_mul(hq, &s_x[IX_Q], qt);
                for (n2 = 0u; n2 < 4u; n2++) s_x[IX_Q + n2] = qt[n2];
                q_norm(&s_x[IX_Q]);
            }
        }
        if (st == 0u) s_gate_bits |= V5F_EKF_GB_MAG;"""
t = t.replace(a, b, 1)
assert t.count('/*') == t.count('*/') and t.count('{') == t.count('}')
sw(P, t, 'gbk', 's2e')

c = open(P, 'rb').read().decode('gbk')
ver = int(re.search(r'#define V5F_FW_VER\s+(\d+)u', open(T, 'rb').read().decode('gbk')).group(1))
nch = int(re.search(r'#define JF_CH_NUM\s+(\d+)u',
                    open(R + r'\V5F\User\src\SPI_rx.c', 'rb').read().decode('gbk')).group(1))
for k, v in [('VER=36', ver == 36), ('限幅在', 'V5F_EKF_YAW_STEP_MAX_DEG * DEG2RAD' in c),
             ('q_yaw 工具在', 'static float q_yaw' in c),
             ('花括号平衡', c.count('{') == c.count('}')),
             ('注释配平', c.count('/*') == c.count('*/'))]:
    print('  %-14s %s' % (k, v))
    assert v, k
print()
print('fw_tag 期望 = %d' % ((ver << 16) | (nch << 8) | 1 | 2 | 4))
