...
__attribute__((aligned(4))) uint8_t USBHS_EP2_Rx_Buf[DEF_USBD_HS_PACK_SIZE];  /* CDC bulk OUT(来即丢) */
__attribute__((aligned(4))) uint8_t USBHS_EP2_Tx_Buf[DEF_USBD_HS_PACK_SIZE];  /* CDC bulk IN:数据上行 */
__attribute__((aligned(4))) uint8_t USBHS_EP3_Tx_Buf[DEF_USB_EP3_HS_SIZE];    /* CDC 通知 interrupt IN */
...
{
    /* CDC/ACM:EP0(ctrl) + EP2(数据 bulk IN/OUT) + EP3(通信通知 interrupt IN) */
    ...
    USBHSD->UEP2_RX_DMA = (uint32_t)(uint8_t *)USBHS_EP2_Rx_Buf;   /* 主机→设备 bulk OUT(EIO?)丢弃/预留 */
    ...
    USBHSD->UEP2_RX_CTRL = USBHS_UEP_R_RES_ACK;                    /* OUT 随时 ACK(来即收,丢弃) */
    USBHSD->UEP2_TX_CTRL = USBHS_UEP_T_RES_NAK;
    USBHSD->UEP3_TX_CTRL = USBHS_UEP_T_RES_NAK;                    /* notify:主机多数不轮询,保持 NAK */
}
                ...
                if((USBHSD->UEP0_RX_CTRL & USBHS_UEP_R_SETUP_IS) && !(USBHSD->UEP0_RX_CTRL & USBHS_UEP_R_DONE))
                {
                    ...
                    if((USBHS_SetupReqType & USB_REQ_TYP_MASK) != USB_REQ_TYP_STANDARD)
                    {
                        ...
                        if((USBHS_SetupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS)
                        {
                            ...
                            {
                            /* ---- CDC/ACM 通信类请求(设备侧仅需要“不报错”收/回) ---- */
                            case CDC_SET_LINE_CODING:      /* 0x20 SET_LINE_CODING(OUT 数据 7B),ACK 即可 */
                            case CDC_SET_CTRL_LINE_STATE:  /* 0x22 */
                                /* 载荷由数据阶段处理(downstream case CDC_SET_LINE_CODING),此处正常返回 */
                                break;

                            case CDC_GET_LINE_CODING:      /* 0x21: 主机查询当前配置(回 7B:1200/8N1) */
                                {
                                    static const uint8_t lg[7]={0xE0,0x04,0,0,/*dwDTERate 1200*/0,0,0};
                                    len = 7;
                                    memcpy(USBHS_EP0_Buf, lg, 7);
                                }
                                break;
                ...
                else
                {
                    ...
                    if((USBHS_SetupReqType & USB_REQ_TYP_MASK) != USB_REQ_TYP_STANDARD)
                    {
                        if((USBHS_SetupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS)
                        {
                            ...
                            {
                                /* 主机下发的 7B line coding:虚拟串口固定 8N1,仅消费不清存 */
                                USBHS_SetupReqLen -= len;
        ...
        else
        {
            ...
            case DEF_UEP0:
                ...
                if((USBHS_SetupReqType & USB_REQ_TYP_MASK) != USB_REQ_TYP_STANDARD)
                {
                    ...
                    if((USBHS_SetupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS)
                    {
                        /* CDC GET_LINE_CODING(7B) 已在 SETUP 阶段回满,这里无需再发数据 */
                    }
                }
                else
                {
            ...
            case DEF_UEP2:
                ...
                /* 发送(下一包装载)统一由主循环 hid_up_flush() 完成：
                   单一消费者无并发，配合上行 8192B 缓冲足够消化。 */
                break;
            default:
                break;
            }
