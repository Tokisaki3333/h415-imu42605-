 ...
 *                共享区所有通道（陀螺/磁力计/气压计/GPS）现均由 V3F 用 GetTime64_10Ns() 写入，
 *                故这里原样搬运、不做任何换算，可直接与 GetTime64_10Ns() 比较。
 *                跨核读取一律按"检测到 cnt 变化 → acquire 屏障 → 再读数据/ts"进行（见 SPI_rx.c 的 hold_poll）。 *   new_data  —— 新数据信号：写入方（DMA 中断）搬完新数据后置 1，读取方处理完自行清 0。
