...
static float    s_am_err;       /* |a|-1，加计模值偏差 -> 倾角观测的附加方差 */    /* 被判为瞬时跳变而丢弃的计数（诊断）*/
...
{
    /* ★VER=47 死循环防护：a = +/-Inf 时 a -/+ TWOPI 仍是 Inf，
...
static void ekf_m6_tilt(const volatile v5f_hold_t *h, const volatile v5f_proc_gate_t *gate)
{
    ...
    R[0] = R[4] = R[8] = V5F_EKF_TILT_SIG_RAD * V5F_EKF_TILT_SIG_RAD + s_am_err * s_am_err;
    {
        ...
        st = /* ★VER=48 掩码 0x7FC0 -> 0x7EC0：去掉 bit8(偏航)。
...
static void ekf_m7_mag(const volatile v5f_hold_t *h, const volatile v5f_proc_gate_t *gate)
{
     ...
     *     留在 R 里就是重复计数，等于把信息白白扔掉。 */
    {
        ...
        s_mag_bh = sqrtf(bh2);          /* |B| = 1（mag_f 是单位矢量）*/
        ...
        /* 倾角列保留在 H 里（新息要能预测倾斜的贡献），但掩码只放行第 8 项：
    ...
    s_mag_r = r[0] * RAD2DEG;
    /* ★VER=45 大新息必须按**性质**分开，否则偏航永远牵不到磁北：
